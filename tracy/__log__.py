
{'date': 'Fri Jul 02 2021 15:11:46.129 GMt-0300 (Horário Padrão de Brasília) -X- SuPyGirls -X-',
'error': '''Traceback (most recent call last):
  module _core.main line 180
    dialog.action(lambda *_: self.start()
  module _core.supygirls_factory line 135
    self.act(self, lambda *_: self.hide() or extra()) if self.act else None
  module _core.supygirls_factory line 310
    return self._first_response(lambda: self._executa_acao(), self.extra, self.error)
  module _core.supygirls_factory line 282
    traceback.print_exc(file=sys.stderr)
  module _core.supygirls_factory line 299
    exec(self.code, glob)  # dict(__name__="__main__"))
  module <module> line 127
    Fase3()
  module <module> line 50
    self.maria.fala()
TypeError: 'str' object is not callable
'''},
{'date': 'Fri Jul 02 2021 15:52:37.50 GMt-0300 (Horário Padrão de Brasília) -X- SuPyGirls -X-',
'error': '''
 module <string> line 160
  self.maria.afala=(self.parede,"Marque os processos que o DNA executa para a produção de proteína",foi=self.anda).vai()
                                                                                                                  ^
SyntaxError: invalid syntax
'''},
{'date': 'Fri Jul 02 2021 16:18:30.73 GMt-0300 (Horário Padrão de Brasília) -X- SuPyGirls -X-',
'error': '''
 module <string> line 194
  Texto=(self.organela,"Maria, olhe a estrutura",foi=self.maria.fala).vai()
                                                                     ^
SyntaxError: invalid syntax
'''},
{'date': 'Fri Jul 02 2021 16:19:57.712 GMt-0300 (Horário Padrão de Brasília) -X- SuPyGirls -X-',
'error': '''Traceback (most recent call last):
  module _core.main line 180
    dialog.action(lambda *_: self.start()
  module _core.supygirls_factory line 135
    self.act(self, lambda *_: self.hide() or extra()) if self.act else None
  module _core.supygirls_factory line 310
    return self._first_response(lambda: self._executa_acao(), self.extra, self.error)
  module _core.supygirls_factory line 282
    traceback.print_exc(file=sys.stderr)
  module _core.supygirls_factory line 299
    exec(self.code, glob)  # dict(__name__="__main__"))
  module <module> line 223
    Fase5()
  module <module> line 190
    self.complexog=Personagem(img=COMPLEXOPG, afala="Sou uma organela, ué",responde=self.mariafala)
NameError: name 'COMPLEXOPG' is not defined
'''},